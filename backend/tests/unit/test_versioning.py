...
class TestGitVersioningInit:
    ...
    def test_init_runs_git_commands(self, mock_run):
        """测试 init 执行 git init 和 config"""
        workspace = Mock()
...
class TestGitVersioningCommit:
    ...
    @patch("subprocess.run")
    def test_commit_stages_and_commits(self, mock_run):
        """测试 commit 暂存并提交"""
        mock_head = Mock()
...
class TestGitVersioningLog:
    ...
    @patch("subprocess.run")
    def test_log_returns_commit_list(self, mock_run):
        """测试 log 返回提交列表"""
        mock_result = Mock()
    ...
    @patch("subprocess.run")
    def test_log_empty_repo(self, mock_run):
        """测试空仓库返回空日志"""
        mock_result = Mock()
        mock_result.stdout = ""
        mock_run.return_value = mock_result
